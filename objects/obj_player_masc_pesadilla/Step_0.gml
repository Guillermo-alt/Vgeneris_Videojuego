...
if(sal && !place_free(x,y+1)){
	...
	if(sprite_index==spr_player_boy_walk_right){
	sprite_index = spr_player_boy_walk_right;}
	
		if(sprite_index==spr_player_boy_walk_left){
	sprite_index = spr_player_boy_walk_left;}
...
if(!instance_exists(obj_conversacion_player_pesadilla1) && globo==false
&& !place_free(x,y+1)){
instance_create_depth(x,y-90,0,obj_conversacion_player_pesadilla1);
globo =true;
}